...
if ($rec == 'handle') {
    ...
    $dou_cloud->handle($type, $cloud_id, $mode, $version);
} 
...
elseif ($rec == 'order') {
    ...
    $cloud_html = $dou->curl('https://cloud.dou.co/order/extend_client', $data);
...
elseif ($rec == 'copyright') {
    if ($_USER['action_list'] != 'ALL') {
	   ...
	   // 验证账号密码是否正确.CURL
    $cloud_account = unserialize($_CFG['cloud_account']);
    $data = array('user' => $cloud_account['user'], 'password' => $cloud_account['password'], 'domain' => ROOT_URL, 'shell' => substr(md5(DOU_SHELL), 16), 'version' => $_CFG['douphp_version']);
    $response = $dou->curl('https://cloud.dou.co/copyright', $data);
...
elseif ($rec == 'account_post') {
    ...
    $response = $dou->curl('https://cloud.dou.co/user/client_user_check', $data);
