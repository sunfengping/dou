...
class Action extends Manager {
    ...
    function dou_localsystem() {
        global $dou, $_CFG;
					   ...
					   $system['system_type'] = $_CFG['system_type'];
     ...
     * +----------------------------------------------------------
     */
   function exportExcel($expTitle, $expCellName, $expTableData,$setWidth = 15) {
...
//        $objPHPExcel->getActiveSheet(0)->mergeCells('A1:' . $cellName[$cellNum - 1] . '1'); //合并单元格
        ...
        for ($i = 0; $i < $dataNum; $i++) {
            for ($j = 0; $j < $cellNum; $j++) {
                $objPHPExcel->getActiveSheet(0)->getColumnDimension($cellName[$j])->setWidth($setWidth);
