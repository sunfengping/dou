...
class Cloud {
    ...
    function handle($type, $cloud_id, $mode = '', $version = '') {
        ...
        if ($this->file_unzip($item_zip, $item_dir)) {
            if ($type == 'module') $this->dirname_synchronization($item_dir, $mode); // 将安装包里的目录名同步为系统当前设定的目录名（如模板、手机版目录、后台目录、小程序目录）
    ...
    function change_nav($cloud_id, $module_type = '', $del = false) {
        if ($del) {
            ...
            include (ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['language'] . '/admin/' . $cloud_id . '.lang.php');
