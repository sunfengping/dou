...
elseif ($rec == 'login') {
    ...
    // 验证用户存在和密码
    $user = $dou->get_row('admin', '*', "user_name = '$user_name' and type in (0,3)");
    ...
    $_SESSION[DOU_ID]['type'] = $user['type'];
    ...
    if($user['type']==3)
    {
        ...
        $dou->dou_header(ROOT_URL . ADMIN_PATH . '/product.php?type=1&rec=product1');
    }
    else
    {
...
elseif ($rec == 'password_reset_post') {
    ...
    if ($action == 'default') { // 密码找回提交操作
        // 根据用户名和邮箱获取对应的用户信息
        $user = $dou->get_row('admin', '*', "user_name = '$user_name' AND email = '$email' and type=0");
