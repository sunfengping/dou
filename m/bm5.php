...
$counts = $dou->fn_query("SELECT COUNT(*) as counts from 58_list where pid=".$id." and bmtype=0 ");
...
if(count($flag)>0){
	...
	if($userinfo[0]["truename"]!=null&&$userinfo[0]["truename"]!="null"&&$userinfo[0]["truename"]!="" &&$userinfo[0]["card_id"]!=null&&$userinfo[0]["card_id"]!="null"&&$userinfo[0]["card_id"]!=""){
			...
			if((int)$product[0]["max"]>(int)$counts[0]["counts"]){
				 if($stop_time[0]["stop_time"]>=date('Y-m-d H:i:s',time())){
					 if($productflag!=null&&$productflag!='null'){
							$arr = $dou->query("INSERT INTO 58_list (username,pid,user_id,add_time,status,pay_money,card_id,ppid)VALUES ('".$name."',0,".$userid.",'".$addtime."',0,".$money.",'".$cardid."',".$id.")");
							...
							$arr = $dou->query("INSERT INTO 58_list (username,pid,user_id,add_time,status,pay_money,card_id,ppid,bmtype)VALUES ('".$name."',0,".$userid.",'".$addtime."',0,".$money.",'".$cardid."',".$id.",1)");
