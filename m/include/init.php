...
if (defined('ROUTE')) { // 伪静态入口（会包含include目录名）
    ...
    define('M_URL', str_replace('include/', '', $dirname));
} else { // 直接访问
    include_once ('../data/config.php');
    define('M_URL', $dirname);
}
...
if (!defined('EXIT_INIT')) {
    ...
    $_LANG['copyright'] = preg_replace('/d%/Ums', $_CFG['site_name'], $_LANG['copyright']);
	   ...
	   if (file_exists($cdkey_file = ROOT_PATH . 'data/..cdkey.php')) {
					   ...
					   $decompile_init = $dou->decompile($cdkey);
					
					   if ($decompile_init == substr(md5(DOU_SHELL), 16) . $_CFG['douphp_version']) {
									   $_LANG['powered_by'] = '';
								}
				}
