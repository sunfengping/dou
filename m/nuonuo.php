...
// if ($rec == 'auth') {
    ...
    $url = "https://auth.nuonuo.com/auth/v1/apply.do";//test
    //加密参数访问诺诺接口
    ...
    //便利解析诺诺地址，找到h5链接
    $json_reuslt=json_decode($reuslt, true);
   
    $data_reuslt=$json_reuslt['data'];
     
    $weburl_json=base64_decode($data_reuslt);
    $weburl_json = $aes->decrypt($weburl_json);
    
    $weburl=json_decode($weburl_json, true);
    $h5url =$weburl['h5'];

    $arr = array('h5url'=>$h5url); 
